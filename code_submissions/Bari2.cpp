...
struct Cone : public Intersectable {
	...
	Cone(const vec3& _baseCenter, vec3 _axisDirection, float _coneAngle, float _height, Material* _material) {
	...
	Hit intersect(const Ray& ray) {
		...
		float cosineSquared = (float)cos(coneAngle) * (float)cos(coneAngle);

		vec3 originToBase = rayOrigin - baseCenter;

		float quadraticA = powf(dot(rayDirection, axisDirection), 2) - dot(rayDirection, rayDirection) * cosineSquared;

		float quadraticB = 2 * dot(rayDirection, axisDirection) * dot(originToBase, axisDirection) - 2 * dot(rayDirection, originToBase) * cosineSquared;

		float quadraticC = powf(dot(originToBase, axisDirection), 2) - dot(originToBase, originToBase) * cosineSquared;
		...
		hit.t = (t2 > 0) ? t1 : t2;
		hit.position = rayOrigin + rayDirection * hit.t;

		if (dot(hit.position - baseCenter, axisDirection) > height || dot(hit.position - baseCenter, axisDirection) < 0) {
			return Hit();
		}

		hit.normal = normalize(2 * dot(hit.position - baseCenter, axisDirection) * axisDirection - 2 * (hit.position - baseCenter) * cosineSquared);
		hit.material = material;
		return hit;
	}
};
...
#pragma omp parallel for
	...
	vec3 refract(vec3 incidenceVec, vec3 normalVec, float refractionRatio) {
		float cosIncidence = -dot(normalVec, incidenceVec);
		float discriminant = 1 - (1 - cosIncidence * cosIncidence) / refractionRatio / refractionRatio;
		if (discriminant < 0) return vec3(0, 0, 0);
		return incidenceVec / refractionRatio + normalVec * (cosIncidence / refractionRatio - sqrtf(discriminant));
	}
};
